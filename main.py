    ...
    while True:
        ...
        if event == 'Save_data':
            ...
            while True:
                ...
                if event == 'Save':
                    # Guardo los archivos.
                    window, event, path = save.save(window, data, indx, value['Start'], value['End'])

                if event == 'Graficar':                    
                    event = 'Plot'
                    del start, end, cargar, holes_size, indx, holes, num_csv_per_sensor, data
                    del ii, val, value
            ...
            if event == 'CSV':
                ...
                while True:
                    if event == 'Extraction':   
                        window, event, value, minimum, maximum = Csv.csv_files(window, path)
                    ...
                    if event == 'Sensor_info':
                        ...
                        key = True
                        while key:
                            window, event, value, indx_raw, key = Csv.sensors_info(list(csv_data_raw.keys()),window)
                        del key
                    ...
                    if event == 'SensorDistribution':
                        ...
                        key = True
                        while key:
                            window, event, indx, carretera_lateral, key, rows_sen = Csv.distribution(window,indx_raw,rows,columns)
                        del key
